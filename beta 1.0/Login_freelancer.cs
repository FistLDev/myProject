...
namespace beta_1._0
{
    public partial class Login_freelancer : Form
    {
        Point lastpoint, lastpoint_logo;//переменные с координатами
        public Login_freelancer(Form f)
        {
        ...
        private void Login_freelancer_MouseDown(object sender, MouseEventArgs e)
        {
            lastpoint.X = e.X;
            lastpoint.Y = e.Y;
        }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            using (SqlConnection conn = DBUtils.GetDBconnection())
            {
                ...
                conn.Open();
                string sqlExpression = "SELECT * FROM Freelancer WHERE Email = '" + textBox2.Text + "' AND Pass = '" + textBox3.Text + "' ";
                string sqlExpression2 = "SELECT Username FROM Freelancer WHERE Email = '" + textBox2.Text + "' AND Pass = '" + textBox3.Text + "' ";
                ...
                if (table.Rows.Count > 0)//проверяем, есть ли в таблице данные
                {
                    ...
                    Personal_acc_freelancer perAcc = new Personal_acc_freelancer(this, username);
                    perAcc.Show();
                    this.Close();
                    conn.Close();
                }
        ...
        private void Login_freelancer_MouseMove(object sender, MouseEventArgs e)
        {
            if (e.Button == MouseButtons.Left)
            {
